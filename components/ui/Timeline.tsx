    ...
    <div className="relative mt-10">
      ...
      {entries.map((entry) => (
        <Reveal key={entry.year} className="relative pb-[34px] pl-11">
          ...
          <span className="font-mono text-[13px] text-bronze tracking-[0.14em] font-semibold">{entry.year}</span>
          <h3 className="text-lg my-1.5">{entry.title}</h3>
          <p className="text-muted text-[14.5px] max-w-[62ch]">{entry.body}</p>
        </Reveal>
      ))}
    </div>
  );
